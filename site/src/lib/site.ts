...
export const names: Record<string,string> = {kin:'KIN',spps:'SPPS','qq-lingxi':'QQ 灵犀',pet:'PET'};
export async function siteContent() {
  ...
  const cases = view.projects.filter(p=>p.id!=='teaching').map(p=>({...p,sections:caseSections(p.id,entries.find(e=>e.data.id===p.id)!.body!)}));
