...
const output=path.resolve(root,outputArg>=0?process.argv[outputArg+1]:'delivery/audits/R03/attempt-01');
...
const server=http.createServer((req,res)=>{
 ...
 res.writeHead(200,{'Content-Type':file.endsWith('.css')?'text/css':'text/html; charset=utf-8'}).end(fs.readFileSync(file));
});
...
try {
 for(const [name,route] of routes) for(const width of [1440,768,375,320]) for(const js of [true,false]) {
   ...
   for(const field of ['phone','birth_year_month'])assert.ok(!text.includes(data.profile.fields[field]));
   ...
   assert.equal(await page.locator('a[download],a[href$=".pdf"]').count(),0);
   if(name==='home') {
 ...
 for(const href of localLinks){await page.goto('about:blank');const url=new URL(href);const response=await page.goto(href);assert.equal(response.status(),url.pathname==='/not-a-real-page/'?404:200);if(url.hash)assert.ok(await page.locator(`[id="${url.hash.slice(1)}"]`).count());evidence.local_links.push(url.pathname+url.hash);}
