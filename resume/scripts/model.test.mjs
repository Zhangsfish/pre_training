...
test('JD fixtures change selection/order while homepage is never mutated',()=>{
 const files=['site/src/pages/index.astro','publication/home.json'];
 const before=files.map(f=>fs.readFileSync(root+f,'utf8'));
 ...
 files.forEach((f,i)=>assert.equal(fs.readFileSync(root+f,'utf8'),before[i]));
});
